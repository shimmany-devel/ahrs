			  ...
			  case 0x82: { // Orientation, Euler Angles (0x82, 0x05)
				...
				if(pframe->field_descriptor == 0x05) { // process sensor data
					...
					if(checksum == payload_checksum) {
						sprintf(wr_buffer,"%10.2f, %10.2f, %10.2f\r\n",
								rad2deg(big2little(data->roll)),
								rad2deg(big2little(data->pitch)),
								rad2deg(big2little(data->yaw))
						);
						if(logging) {
							fwrite(wr_buffer,sizeof(char),strlen(wr_buffer),wfp);
						}
						if(toggle_display) {
							printf("%s",wr_buffer);
						}
...
void format_time(char *output){
	...
	sprintf(output, "AHRS-%d_%02d_%02d_%02d%02d%02d.csv",
...
void help(char *appname)
{
	printf("%s Usage:\n", basename(appname));
	printf("\t%s device [1|0]\r\n", basename(appname));
	printf("\t\t: if 3rd param is 1 to create logfile, 0 to display\n");
}
...
int main(int argc, char *argv[])
{
	...
	if(logging) {
		...
		format_time(file_name);
		wfp = fopen(file_name,"w");
		if(wfp==NULL) {
			printf("File %s creation fail!\n", file_name);
			return -1;
		}
	}		
